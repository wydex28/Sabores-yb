...
<div class="max-w-3xl mx-auto">
    <div class="mb-10 text-center">
        ...
        <h2 class="text-4xl font-display text-textMain tracking-tight">Editar {{ $module->name }} #{{ $entry->id }}</h2>
        <p class="text-gray-500 font-medium italic">Actualiza la información del registro de {{ $module->name }}.</p>
    </div>

    <div class="bg-white rounded-[2.5rem] p-10 sm:p-14 shadow-2xl shadow-gray-200/50 border border-gray-100">
        <form action="{{ route('admin.modules.update', [$module->name, $entry->id]) }}" method="POST" class="space-y-8">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div>
                    ...
                    @if($field['type'] == 'text' || $field['type'] == 'textarea')
                        <textarea name="{{ $field['name'] }}" rows="3" class="w-full bg-gray-50 border border-gray-100 rounded-3xl py-4 px-6 focus:ring-4 focus:ring-primary/10 transition-all text-sm font-bold" required>{{ $entry->{$field['name']} }}</textarea>
                    @elseif($field['type'] == 'integer' || $field['type'] == 'decimal')
                        <input type="number" step="any" name="{{ $field['name'] }}" value="{{ $entry->{$field['name']} }}" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 focus:ring-4 focus:ring-primary/10 transition-all text-sm font-bold" required>
                    @else
                        <input type="text" name="{{ $field['name'] }}" value="{{ $entry->{$field['name']} }}" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 focus:ring-4 focus:ring-primary/10 transition-all text-sm font-bold" required>
                    @endif
                </div>
                @endforeach
                        ...
                        <div>
                            ...
                            <select name="{{ $fk }}" class="w-full bg-gray-50 border border-gray-100 rounded-2xl py-4 px-6 text-sm font-bold select2" required>
                                ...
                                @foreach(($relatedData[$rel['model']]['data'] ?? []) as $item)
                                    <option value="{{ $item->id }}" {{ $entry->{$fk} == $item->id ? 'selected' : '' }}>
